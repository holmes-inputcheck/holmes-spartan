...
fn main() -> std::io::Result<()> {
  ...
  let x = 1;
  let num_range_checks = vec![100000, 200000, 500000];

  let num_vars = vec![2097152, 4194304, 4194304, 8388608, 8388608,
    4194304, 8388608, 8388608, 16777216, 16777216,
    16777216, 16777216, 33554432, 33554432, 33554432];  // number of variables generated total in range checks and private data loads (18 (total intermediary variables) * 100000 + 1 (1 total private data load) * 100000)
  
  let num_cons = vec![2097152, 4194304, 4194304, 8388608, 8388608,
    4194304, 8388608, 8388608, 16777216, 16777216,
    16777216, 16777216, 33554432, 33554432, 33554432];  // total number of OT triples generated during range changes (32 (OT generated per range check) * 100000 = 3200000)
  ...
  for num_range_check in num_range_checks {
    let mut output = File::create("range_check_".to_owned() + &num_range_check.to_string() + "_nizk.txt")?;
    writeln!(output, "DecompSize,2party,6party,10party");

    for b in &b_list {
      let times = run_nizk_range(a, *b, x, num_range_check, num_vars[it], num_cons[it], num_inputs);
...
fn run_nizk_range(a: u32, b: u32, x: u32, num_range_checks: usize, num_vars: usize, num_cons: usize, num_inputs: usize) -> Vec<String> {
  ...
  println!("Running {:?} range checks with decomposition size 2^{:?} in NIZK", num_range_checks, size_of_decomp);
  ...
  let proof = NIZK::prove(&inst, assignment_vars, &assignment_inputs, &gens, &mut prover_transcript);
  ...
  {
      let mut first_verifier_transcript = Transcript::new(b"nizk_example");
      let verify_start = Instant::now();
      assert!(proof
      .verify(&inst, &assignment_inputs, &mut first_verifier_transcript, &gens)
      .is_ok());
  ...
  let mut v_duration_6 = Duration::new(0, 0);
  {
      ...
      .map(|mut v| {
          let verify_start = Instant::now();
          assert!(proof
          .verify(&inst, &assignment_inputs, &mut v, &gens)
          .is_ok());
