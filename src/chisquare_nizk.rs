...
fn main() -> std::io::Result<()> {
    ...
    let num_jls = vec![2500, 5000];
    let dim_sizes = vec![(1, 10), (4, 10), (4, 50)];
    let num_vars = vec![1048576 * 2 * 2 * 2, 1048576 *  2 * 2 * 2 * 2]; // number of variables generated total in JL and private data loads
    let num_cons = vec![1048576 * 2 * 2 * 2, 1048576 *  2 * 2 * 2 * 2]; // total number of OT triples generated during JL
    let num_inputs = 4 * k + 2; // total number of public inputs

    let mut out_it = 0;
    for num_jl in num_jls {
        let mut output = File::create("jl_".to_owned() + &num_jl.to_string() + "_nizk.txt")?;
        writeln!(output, "NumDims,SizeOfEachDim,2party,6party,10party");
        
        for it in 0..3 {
          let times = run_nizk_chi(dim_sizes[it].0, dim_sizes[it].1, num_jl, num_vars[out_it], num_cons[out_it], num_inputs);
  ...
  fn run_nizk_chi(num_dims_u32: u32, size_of_each_dimension_u32: u32, num_jls: usize, num_vars: usize, num_cons: usize, num_inputs: usize) -> Vec<String> {
    ...
    println!("Running {:?} JL checks with {:?} dimensions of size {:?} in NIZK", num_jls, num_dims, size_of_each_dimension_u32);
